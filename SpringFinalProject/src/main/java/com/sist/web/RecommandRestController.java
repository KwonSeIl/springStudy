...
public class RecommandRestController {
   ...
   @GetMapping(value="recommand_change.do",produces = "text/plain;charset=UTF-8")
   public String recommand_change(int no)
   {
      String[] strArr=null;
      if(no==1)
      {
         strArr=new String[] {"출퇴근길","휴식","공부","집","카페","휴가","드라이브","산책","운동","집중","거리","클럽","해변","공연","고백"};
      }
      else if(no==2)
      {
         strArr=new String[] {"기분전환","외로움","슬픔","힘찬","이별","지침","설렘","오후","위로","밤","새벽","저녁","아침","사랑","스트레스","그리움","추억","우울","행복"};
      }
      else if(no==3)
      {
         strArr=new String[] {"밝은","신나는","편안한","따뜻한","그루브한","부드러운","로맨틱한","웅장한","몽환적인","달콤한","시원한","애절한","어두운","발렌타인데이","화이트데이"};
      }
      else if(no==4)
      {
         strArr=new String[] {"봄","여름","가을","겨울","맑은날","추운날","흐린날","비오는날","더운날","안개낀날","눈오는날"};
      }
   ...
   public String recommand_result(String ss)
   {
      ...
      for(String s:list)
      {
         for(int i=0;i<m.length;i++)
         {
            ...
            {
               String data=m[i].group(); //찾은 문자열 가지고 오기
               count[i]++;
            }
         }
      }
